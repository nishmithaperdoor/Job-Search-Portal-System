...
mongoose.connect('mongodb://localhost:27017/jobcase');
...
var blogSchema = new Schema({
    ...
    phone: Number,
    usertype: String,
    job:  [{ jobtitle: String,jobcompany: String ,jobdescription: String, keywords: String, location: String }]
});

var jobcase = mongoose.model('jobcase', blogSchema);
...
app.post('/postData',function(req,res){
    ...
    jobcase.findOne({ username: item.username, email: item.email}, function(err, doc) {
        if (err) {
            console.log(err);
        }
        if(!doc){
            var data = new jobcase(item);
            data.save();
            res.json({A: "yes"});
        }
        else{
...
app.post('/login',function(req,res){
    ...
    jobcase.findOne({ username: item.username, password: item.password}, function(err, doc) {
        if (err) {
            console.log(err);
        }
        if(!doc){
...
app.post('/checkUserType', function(req, res){
    jobcase.findOne({ username: req.session.user }, function (err, doc) {
        if(err)
            console.log(err);
...
app.post("/getJobs", function (req, res) {
    var query = jobcase.find().select('job');
